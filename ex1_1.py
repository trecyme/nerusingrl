...
def experiment1_1_train(output_folder,
                      word_vectors,
                      n_episodes=300,
                      trainset_path='./conll2003/en/eng.train',
                      ):
    # 初始化环境
    print('[' + util.now_time() + "] init environment...")
    env = Env1(trainset_path, word_vectors)
    ...
    for i in range(n_episodes):
        ...
        while True:
            ...
            if env.end():
                ...
                test_eval = experiment1_1_test(output_folder, word_vectors, agent, i)
                break
...
def experiment1_1_test(output_folder,
                     word_vectors,
                     agent,
                     episode_index,
                     testset_path='./conll2003/en/eng.testb',
                     ):
    # 初始化环境
    env = Env1(testset_path, word_vectors)
    step = 0
    s = env.reset()
...
if __name__ == "__main__":
    ...
    # 初始化词向量，作为全局变量
    # embeding_path = './word2vec/GoogleNews-vectors-negative300.bin.gz'
    embeding_path = './word2vec/glove.6B.300d.txt'
    print('[' + util.now_time() + "] 初始化词向量...")
    word_vectors = dp.load_glove_word2vec(embeding_path)
    print('[' + util.now_time() + "] 词向量初始化完毕")

    # 训练模型
    experiment1_1_train(output_folder, word_vectors, n_episodes=3)
